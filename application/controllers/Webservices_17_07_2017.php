 ...
 // Allow from any origin
	...
	class Webservices extends CI_Controller{
    	...
    	if($result['status']=='success'){
    		$result = array('status'=>"success",'data'=>array('auth_token'=>$result['auth_token'],'user_id'=>$result['user_id'],'is_phone_verified'=>$result['is_phone_verified']));
    	} else {   		
	...
	public function trip_details($id=null){
		...
		if(isset($request['id'])){
			...
			if(count($result)>0){
				...
				$rate = $this->Webservice_model->driver_rate($result->driver_id);
				$result->rating = $rate;
				print json_encode(array('status' =>'success' ,'data'=>$result));
...
// 	      $subject = 'LaTaxi';
// 	      $name= 'Doan antony';
// 	      $mailTemplate= 'Your new password:'.$result;
